...
def read_science_masks_from_hdul(hdul, spatial_shape):
    """Read science masks from an already-open FITS container.

    Keeping this small primitive separate lets alignment consume pixels,
    headers and masks in one FITS open.  The path-based public helper below
    retains the old API for callers that do not already own an HDUList.
    """
...
def read_science_masks(path, spatial_shape):
    with fits.open(path, memmap=False, lazy_load_hdus=True) as hdul:
        return read_science_masks_from_hdul(hdul, spatial_shape)
